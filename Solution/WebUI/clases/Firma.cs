...
namespace WebUI
{
    public class Firma
    {
        ...
        public static void sign(string path)
        {
            ...
            X509Certificate certificate = LoadCertificate(pathfirma + "\\diego_adrian_pineda_espinoza.p12", "Barcelona", out privateKey, out provider);
            //X509Certificate certificate = LoadCertificate("c:\\temp\\certificado.pfx", "contraseña", out privateKey, out provider);
        ...
        public static void signxml(string xmlpath, string clave, string certificado, string password, string pathcertificado)
        {
            ...
            string pathfirma = System.Configuration.ConfigurationSettings.AppSettings["path"];
            if (string.IsNullOrEmpty(pathfirma))
                pathfirma = HttpContext.Current.Server.MapPath("../" + pathcertificado);
            else
                pathfirma += "\\" + pathcertificado;

            
            
            //string pathfirma = HttpContext.Current.Server.MapPath("../firmas");

            X509Certificate certificate = LoadCertificate(pathfirma + "\\"+certificado, password, out privateKey, out provider);
            //Si encontramos el certificado...  
            if (certificate != null)
            {
                //Política de firma (Con las librerías JAVA, esto se define en tiempo de ejecución)  
                ...
                dataToSign.setDocument(LoadXML(xmlpath));
                //Crear datos a firmar  
                /*DataToSign dataToSign = new DataToSign();
                dataToSign.setXadesFormat(EnumFormatoFirma.XAdES_BES); //XAdES-EPES                                  
                dataToSign.setEsquema(XAdESSchemas.XAdES_132);
                ...
                FileOutputStream a = new FileOutputStream(xmlpath); 
